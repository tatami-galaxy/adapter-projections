...
class XLMRobertaAdapterModel(RobertaAdapterModel):
    ...
    def disable_adapter_projection_disjoint(self):
        for layer_i in range(self.config.num_hidden_layers):
            if self.roberta.encoder.layer[layer_i].output.disjoint_projection_flag:
                self.roberta.encoder.layer[layer_i].output.disjoint_projection_flag = False
